...
class Solution:
    def isSubtree(self, root: Optional[TreeNode], subRoot: Optional[TreeNode]) -> bool:
        ...
        while stack:
            ...
            # if the value of the node is the same as the subroot, check the rest of the tree
            if node.val == subRoot.val:
                # if true, then that subtree is in the main tree
                if self.isSameTree(node, subRoot):
                    return True

            # if the children are not null, add them to the stack
            if node.left:
